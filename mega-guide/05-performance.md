...
[← Dependencies](./04-dependencies.md) | [Index](./README.md) | [Profiling →](./06-profiling.md)
...
```
Any heavy JS work (API calls, saga processing) causes animation jank — a leading cause of perceived sluggishness in [ANR analysis](./01-monitoring.md).
...
**Shopify's experience**: At scale, Reanimated caused severe frame drops during [New Architecture migration](./03-new-architecture.md#common-pitfalls). Ensure you're on latest Reanimated before migrating.

**Discord's experience**: They moved drawer interactions to `react-native-gesture-handler` + Reanimated — "buttery smooth" result. Also found a spinning animation consuming 10% CPU while hidden — removing it was a bigger win than any optimization. See [Profiling: Discord Wins](./06-profiling.md#discord-wins).
...
> **See also**: [Profiling: Finding Slow Re-renders](./06-profiling.md#finding-slow-re-renders) for diagnosing animation-related jank
...
**React Compiler (Expo SDK 54+)**: Auto-memoizes at compile time — eliminates this entire class of bugs. See [Profiling: React Compiler](./06-profiling.md#react-compiler).

> **See also**: [Profiling: Finding Slow Re-renders](./06-profiling.md#finding-slow-re-renders) for when memoization actually matters
...
- [ ] Profile with [Flashlight](./06-profiling.md#flashlight)

> **See also**: [Profiling: Flashlight](./06-profiling.md#flashlight) for measuring list FPS | [Dependencies](./04-dependencies.md) for FastImage vs standard Image
...
- **Expensify's approach**: They built [react-native-onyx](https://github.com/Expensify/react-native-onyx) — an offline-first state layer. See [Reading List](./07-reading-list.md).
...
A floating dev-only button for real-time API inspection. Useful when [ANR analysis](./01-monitoring.md) points to network-related freezes. Gate behind `__DEV__`:
...
- [ ] No new [unnecessary dependencies](./04-dependencies.md#before-adding)
- [ ] API fetching uses caching (React Query or equivalent)
- [ ] Tested on a low-end device (not just emulator)
- [ ] [ANR rates](./01-monitoring.md) monitored post-release

---

**Next**: [Profiling & Debugging →](./06-profiling.md)
